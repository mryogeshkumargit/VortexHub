...
class ApiHandler:
    ...
    def _post(self, endpoint_key, payload):
        url = self.ENDPOINTS.get(endpoint_key, endpoint_key)  # Support dynamic endpoints
    ...
    def _post(self, endpoint_key, payload):
        ...
        # The API key is in the header, but some endpoints might expect it in the body.
        # The user's example shows a 'key' field. We will add our key to the payload.
        payload['key'] = self.API_KEY
        response = self.session.post(url, headers=self.headers, data=json.dumps(payload), timeout=30)
        response.raise_for_status()
        return response.json()
    ...
    def fetch_models_from_api(self):
        payload = {"limit": 500} # Key is added in _post
...
class ModelsLabGUI:
    ...
    def load_image_from_url(self, url, label_to_update):
        def task():
            try:
                ...
                label_to_update.image = image # Keep a reference
            except Exception as e:
    ...
    def create_model_tab(self, title, tab_key):
        ...
        # Note: Scrollbars only appear when content exceeds the visible area of the Treeview.
        ...
        right_panel.pack_propagate(False) # Prevent resizing
    ...
    def create_audio_tab(self):
        ...
        audio_types = ["text_to_audio", "text_to_speech"]
    ...
    def load_models_from_db(self):
        ...
        # Categorize models loaded from DB
    ...
    def fetch_models(self):
        ...
        try:
            ...
            self.data_manager.save_models(self.models) # Save categorized models
    ...
    def filter_and_update_treeview(self, tab_key):
        ...
        used_iids = set()  # Keep track of iids used in this tree
        ...
        for model in models:
            ...
            # ** FIX STARTS HERE **
            # Ensure the iid is unique for the Treeview.
            item_id = model.get('model_id')
            
            # If the model_id is missing, empty, or already used, create a new unique one.
            if not item_id or item_id in used_iids:
                # Using the object's memory address via id() guarantees session-wide uniqueness.
                item_id = f"gen_id_{id(model)}"
            
            # This check is a safeguard; with the logic above, it should not be triggered.
            ...
            # The 'values' tuple determines what is displayed in the columns. It can have the original ID.
            values = (
            ...
            # Use the guaranteed-unique 'item_id' for the internal 'iid'.
    ...
    def open_screenshot_url(self, event):
        ...
        # Find the model using the unique iid, which might be a generated one
        model = None
    ...
    def on_model_select(self, event, tab_key):
        ...
        if tab_key == "image":
            ...
            if not self.use_lora_var.get(): # Auto-enable LoRA on selection
    ...
    def generate_audio_task(self):
        try:
            ...
            if not prompt_text:
                messagebox.showerror("Error", "Prompt field cannot be empty.")
                return

            payload = {
                "model_id": model.get('model_id'),
                "prompt": prompt_text,
                "language": "english",  # Added from example
                "webhook": None,       # Added from example
                "track_id": None       # Added from example
            }
            if audio_type == 'text_to_speech':
                payload['voice_id'] = 'default'  # Kept for text_to_speech compatibility
            ...
            if response.get("status") == "success" and response.get("output"):
                audio_url = response["output"][0]  # Updated to match example response
...
if __name__ == "__main__":
    try:
        logger.info("Starting ModelsLab GUI...")
        root = tk.Tk()
